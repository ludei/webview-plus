...
import android.webkit.PublicWebSettings;


public class LudeiWebSettingsProxy extends PublicWebSettings {


    private LudeiWebSettings mAwSettings;

    private LudeiWebSettingsProxy(LudeiWebSettings awSettings) {
        mAwSettings = awSettings;
    }
    
    private static native Object nativeCreateLudeiWebSettings(Context context);
    
    public static LudeiWebSettingsProxy Create(Context context, LudeiWebSettings awSettings) {
    	
    	Constructor<?>[] constructors = android.webkit.WebSettings.class.getDeclaredConstructors();
    	if (constructors.length > 0 && constructors[0].getParameterTypes().length == 2) {
    		//Some Android APIs have a problematic constructor, 2 params and package access
    		//Use JNI to bypass the Java Rules ;)
    		LudeiWebSettingsProxy proxy = (LudeiWebSettingsProxy)LudeiWebSettings.nativeCreateLudeiWebSettingsProxy(context);
    		proxy.mAwSettings = awSettings;
    		return proxy;
    	}
    	else {
    		return new LudeiWebSettingsProxy(awSettings);
    	}	
    }

    LudeiWebSettings getAwSettings() {
        return mAwSettings;
    }
    ...
    public void setDefaultZoom(ZoomDensity zoom) {
        if (zoom != ZoomDensity.MEDIUM) {
            Log.w("LudeiWebSettings", "setDefaultZoom not supported, zoom=" + zoom);
        }
    }
    ...
    public synchronized void setUserAgent(int ua) {
        ...
        if (ua == 0) {
            ...
            Log.w("LudeiWebSettings", "setUserAgent not supported, ua=" + ua);
        }
    }
    ...
    public synchronized int getUserAgent() {
        ...
        return LudeiWebSettings.getDefaultUserAgent().equals(getUserAgentString()) ? 0 : -1;
    }
    ...
    public synchronized void setLayoutAlgorithm(LayoutAlgorithm l) {
        ...
        final LudeiWebSettings.LayoutAlgorithm[] chromiumValues = {
                LudeiWebSettings.LayoutAlgorithm.NORMAL,
                LudeiWebSettings.LayoutAlgorithm.SINGLE_COLUMN,
                LudeiWebSettings.LayoutAlgorithm.NARROW_COLUMNS,
                LudeiWebSettings.LayoutAlgorithm.TEXT_AUTOSIZING
        };
