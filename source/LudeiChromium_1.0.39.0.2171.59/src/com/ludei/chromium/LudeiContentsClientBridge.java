...
public class LudeiContentsClientBridge {
    ...
    private void handleJsAlert(String url, String message, int id) {
		...
		LudeiJsResult ludeiResult = LudeiJsResultHandler.createJsResult(this, id);
		if (mWebChromeClient != null) {
			JsResult result = LudeiWebkitUtils.makeJsResult(contentView, ludeiResult);
			handled = mWebChromeClient.onJsAlert(contentView, contentView.getUrl(), message, result);
		}
		
		if (!handled) {
			mDefaultWebChromeClient.onJsAlert(contentView, url, message, ludeiResult);
		}
    }
    ...
    private void handleJsConfirm(String url, String message, int id) {
		...
		if (mWebChromeClient != null) {
			JsResult result = LudeiWebkitUtils.makeJsResult(contentView, ludeiResult);
			handled = mWebChromeClient.onJsConfirm(contentView, contentView.getUrl(), message, result);
		}
		
		if (!handled) {
			mDefaultWebChromeClient.onJsConfirm(contentView, url, message, ludeiResult);
		}
    }
    ...
    private void handleJsPrompt(String url, String message, String defaultValue, int id) {
		boolean handled = false;
		LudeiJsPromptResult ludeiResult = LudeiJsResultHandler.createJsPromptResult(this, id);
		if (mWebChromeClient != null) {
			JsPromptResult result = LudeiWebkitUtils.makeJsPromptResult(contentView, ludeiResult);
			handled = mWebChromeClient.onJsPrompt(contentView, contentView.getUrl(), message, defaultValue, result);
		}
		
		if (!handled) {
		    mDefaultWebChromeClient.onJsPrompt(contentView, url, message, defaultValue, ludeiResult);
		}
    }
